...
// 'ink' is the colour of the letter sitting on top of 'colour'.
// ============================================================
const CREW = [
  { tag: 'B',   name: 'Ben',    colour: '#C09024', ink: '#1B1D19' },
  { tag: 'J',   name: 'Jerome', colour: '#5E8C56', ink: '#E7E8E1' },
  { tag: 'B&J', name: 'Both',   colour: '#3E6FA8', ink: '#E7E8E1' }
];
...
function crewColour(tag) {
  ...
  return entry ? entry.colour : '#8A8D82';
}
...
function crewTag(tag) {
  const entry = crewEntry(tag);
  if (!entry) return '';
  return `<span class="crewtag" style="background:${entry.colour};color:${entry.ink}">${escapeText(entry.tag)}</span>`;
}
...
function renderCalendar() {
  ...
  el('callegend').innerHTML = CREW
    .map((c) => `<div><span style="background:${c.colour}"></span>${escapeText(c.tag)} &mdash; ${escapeText(c.name)}</div>`)
    .join('');
...
function buildCrewChips() {
  ...
  el('crewchips').innerHTML = anyChip + CREW.map((c) => `
    <button type="button" class="chip${c.tag === crewFilter ? ' on' : ''}" data-crew="${escapeAttr(c.tag)}">
      <span class="swatch" style="background:${c.colour}"></span>${escapeText(c.tag)}</button>`).join('');
}
